// calculation example using a datafile outputed by 'mie_mmls_solver'.
...
int main(int argc,char *argv[]) 
{
  ...
  read_dat_mmls(argv[1],&msp); // read data file
  print_data_mmls(&msp);       // print data
  print_data_mmls_mksa(&msp);  // print data in MKSA system of units
  
  r[0]= 0.5; // set x-coordinate 
  r[1]= 0.0; // set y-coordinate
  r[2]=-1.0; // set z-coordinate
  total_EH_mmls(e,h,r,&msp); // calclation of total field ( add incident field to scattered field )
...
int force_torque_integral(int id,double *vf,double *vn,MSPD *msp)
{
  ...
  rc=msp->sp[id].a[msp->sp[id].n_l-1]*2.0;
